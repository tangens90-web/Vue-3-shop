<template>
  <div class="grid-card" v-for="product in products" :key="product.id">
    <div
      @mouseover="showRectangleBlock(product.id)"
      @mouseout="closeRectangleBlock(product.id)"
      class="vertical-card mb-3 scaleCard"
      style="border-radius: 15px"
      :style="gorizontalView ? `max-width: 1000px;` : `max-width: 18rem;`"
    >
      <div class="">
        <div :class="gorizontalView ? `image-block col-md-4` : `image-block`">
          <div>
            <img
          ...
          <div class="image-block__attributes">
            <div
              class="image-block__rectangle-block flex --just-center rect_block"
...
import type { Product, ImageUrls } from "@/types/product";
...
const checkWishList = (id: number): boolean => {
  ...
  if (wishList.length > 0 && typeof wishList === "string") {
    return JSON.parse(wishList).findIndex((item: Product) => item.id == id) !==
      -1
      ? true
      : false;
  } else {
...
const checkCartItem = (id: number): boolean => {
  ...
  if (cartItem.length > 0 && typeof cartItem === "string")
    return JSON.parse(cartItem).findIndex((item: Product) => item.id == id) !==
      -1
      ? true
      : false;
  else return false;
...
const addToCartItem = (product: Product, event: Event): void => {
  ...
  currentTarget.querySelector<HTMLElement>(".button_text").innerText =
    cartItemText[1];
...
.scaleCard {
  &:hover {
    box-shadow: 8px 10px 5px 2px rgba(0, 0, 255, 0.2);
  }
  .rect_block {
    opacity: 1;
  }
